...
IRResultType TransverseReinfConstraint :: initializeFrom(InputRecord *ir)
{
    IRResultType result;
...
{
    IntArray lambda_loc;  // For the displacements and stress respectively
    ...
    if ( type == ExternalForcesVector ) {
        ...
        if ( boundaries.giveSize() == elements.giveSize()*2 ) {
            for ( int pos = 1; pos <= elements.giveSize(); ++pos) {
                ...
                IntArray bndNodes;
                ec->giveBoundaryEdgeNodes(bndNodes, boundary);
...
void TransverseReinfConstraint :: computeTangent(FloatMatrix &tangent, TimeStep *tStep)
{
    OOFEM_ERROR("Not implemented");
}


void TransverseReinfConstraint :: giveLambdaLocationArray(IntArray &oCols, const UnknownNumberingScheme &r_s)
{
    lmLambda->giveLocationArray(lmLambdaIds, oCols, r_s);
}
